...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw DemoApiScalarGalaxyInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw DemoApiScalarGalaxyInvalidDataException("`$name` is not set")
            is JsonNull -> throw DemoApiScalarGalaxyInvalidDataException("`$name` is null")
            else ->
                throw DemoApiScalarGalaxyInvalidDataException("`$name` is invalid, received $this")
        }
