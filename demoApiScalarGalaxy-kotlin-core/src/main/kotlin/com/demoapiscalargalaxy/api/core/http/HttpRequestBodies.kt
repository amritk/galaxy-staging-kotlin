...
private fun serializePart(name: String, node: JsonNode): Sequence<Pair<String, InputStream>> =
    when (node.nodeType) {
        ...
        JsonNodeType.ARRAY ->
            // Bytes have no comma-joined form, so an array containing a binary element sends every
            // element as its own part instead.
            if (node.elements().asSequence().any { it.nodeType == JsonNodeType.BINARY })
                        ...
                        node
                            ...
                            .mapNotNull { element ->
                                when (element.nodeType) {
                                    ...
                                    JsonNodeType.OBJECT,
                                    JsonNodeType.POJO ->
                                        throw DemoApiScalarGalaxyInvalidDataException(
