package com.demoapiscalargalaxy.api.client.okhttp

import com.demoapiscalargalaxy.api.core.RequestOptions
import com.demoapiscalargalaxy.api.core.Timeout
...
import com.demoapiscalargalaxy.api.core.http.HttpRequestBody
import com.demoapiscalargalaxy.api.core.http.HttpResponse
import com.demoapiscalargalaxy.api.core.http.ProxyAuthenticator
import com.demoapiscalargalaxy.api.errors.DemoApiScalarGalaxyIoException
...
class OkHttpClient internal constructor(private val okHttpClient: okhttp3.OkHttpClient) :
    ...
    override fun execute(request: HttpRequest, requestOptions: RequestOptions): HttpResponse {
        ...
        return try {
            ...
            throw DemoApiScalarGalaxyIoException("Request failed", e)
        } finally {
            request.body?.close()
        }
    }
...
private fun HttpRequest.toRequest(client: okhttp3.OkHttpClient?): Request {
    ...
    if (client != null) {
        if (!headers.names().contains("X-Scalar-Read-Timeout") && client.readTimeoutMillis != 0) {
            builder.addHeader(
                "X-Scalar-Read-Timeout",
                Duration.ofMillis(client.readTimeoutMillis.toLong()).seconds.toString(),
            )
        }
        if (!headers.names().contains("X-Scalar-Timeout") && client.callTimeoutMillis != 0) {
            builder.addHeader(
                "X-Scalar-Timeout",
